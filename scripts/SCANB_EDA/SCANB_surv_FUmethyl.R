#!/usr/bin/env Rscript
# Script: survival analyses in subsets SCAN-B methylation FU cohort
...
#-------------------
# output paths
outfile_1 <- paste0(output_path,"SCANB_FUmethyl_Modalities_surv_",clin_group,".pdf")
outfile_2 <- paste0(output_path,"SCANB_FUmethyl_Modalities_surv_",clin_group,".txt") 
#-------------------
# storing objects
...
for (group in unique(sub_clinical$Group)[!is.na(unique(sub_clinical$Group))]) {
  # Calculate the event counts for OS, RFi, and DRFi for each treatment group
  events <- lapply(
    list(
      sub_clinical$OS_event[sub_clinical$Group == group],
      sub_clinical$RFi_event[sub_clinical$Group == group],
      sub_clinical$DRFi_event[sub_clinical$Group == group]
    ),
    table
  )
  ...
  # Append the current  group's data to the events list
...
plot <- ggplot(events_combined, aes(x = Outcome, y = Count, fill = as.factor(Event))) +
  ...
  labs(
    title = "Stacked Barplot of Events (OS, RFi, DRFi) by Group",
